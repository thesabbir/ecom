...
async def get_price_trends(
    ...
    trends = df.groupby(df['scraped_at'].dt.date).agg({
        'price': ['mean', 'min', 'max', 'std'],
        'product_id': 'count'
    }).round(2)

    trends.columns = ['avg_price', 'min_price', 'max_price', 'price_std', 'product_count']
...
async def competitive_analysis(category: Optional[str] = None):
    ...
    analysis = df.groupby('site_name').agg({
        'price': ['mean', 'min', 'max'],
    ...
    analysis.columns = ['avg_price', 'min_price', 'max_price', 'avg_discount',
                        'avg_rating', 'total_reviews', 'product_count', 'stock_availability']
...
async def get_demand_indicators(top_n: int = Query(20, ge=5, le=100)):
    ...
    high_demand = df.nlargest(top_n, 'review_count')[
        ['title', 'site_name', 'category', 'price', 'rating', 'review_count']
    ]

    out_of_stock = df[df['in_stock'] == False].groupby('category').size().sort_values(ascending=False).head(10)

    price_drops = df[df['discount_percentage'].notna()].nlargest(top_n, 'discount_percentage')[
    ...
    return {
        ...
        "avg_discount_rate": float(df['discount_percentage'].mean() or 0)
    }
...
async def category_performance():
    ...
    category_stats.columns = ['avg_price', 'median_price', 'avg_rating',
                              'total_reviews', 'avg_discount', 'product_count', 'availability']
    category_stats = category_stats.reset_index()
    category_stats = category_stats.sort_values('product_count', ascending=False)

    return {
        "categories": category_stats.to_dict('records'),
        "top_category": category_stats.iloc[0]['category'] if not category_stats.empty else None,
        "most_expensive_category": category_stats.nlargest(1, 'avg_price').iloc[0]['category'] if not category_stats.empty else None,
        "best_rated_category": category_stats.nlargest(1, 'avg_rating').iloc[0]['category'] if not category_stats.empty else None
    }
...
async def brand_analysis(min_products: int = Query(5, ge=1)):
    ...
    brand_stats.columns = ['avg_price', 'min_price', 'max_price',
                           'avg_rating', 'total_reviews', 'product_count', 'avg_discount']
    ...
    return {
        ...
        "brand_concentration": {
            "top_5_market_share": float(
                brand_df[brand_df['brand'].isin(brand_stats.head(5)['brand'])].shape[0] / len(brand_df) * 100
            )
        }
    }
...
async def find_pricing_opportunities(margin_threshold: float = Query(20.0, ge=0, le=100)):
    df = storage.load_products()
    ...
    opportunities = sorted(opportunities, key=lambda x: x['potential_margin'], reverse=True)
    ...
    preferences = {
        "price_sensitivity": {
            ...
            "discount_importance": float(
                ...
                df['review_count'].sum() * 100
            ) if df['review_count'].sum() > 0 else 0
        },
        "quality_indicators": {
            "min_acceptable_rating": 3.5,
            "avg_rating_purchased": float(
                df[df['review_count'] > df['review_count'].median()]['rating'].mean()
            ) if 'rating' in df else None,
            "review_threshold": int(df['review_count'].median()) if 'review_count' in df else 0
        },
        "popular_categories": df.groupby('category')['review_count'].sum().nlargest(10).to_dict(),
        "popular_brands": df.groupby('brand')['review_count'].sum().nlargest(10).to_dict() if 'brand' in df else {},
        "high_rated_characteristics": {
            "avg_price": float(high_rated['price'].mean()) if not high_rated.empty else None,
            "common_categories": high_rated['category'].value_counts().head(5).to_dict() if not high_rated.empty else {}
        }
    }
