...
class Product(BaseModel):
    ...
    condition: ProductCondition = Field(
        ProductCondition.NEW, description="Product condition"
    )
    ...
    scraped_at: datetime = Field(
        default_factory=datetime.now, description="Scraping timestamp"
    )
...
class CrawlRequest(BaseModel):
    ...
    url: HttpUrl
    site_adapter: Optional[str] = Field(
        None, description="Specific site adapter to use"
    )
    max_pages: int = Field(1, ge=1, le=100, description="Maximum pages to crawl")
    skip_duplicates: bool = Field(
        True, description="Skip products that already exist in storage"
    )
    skip_if_scraped_within_hours: Optional[int] = Field(
        24, description="Skip products scraped within N hours (None to disable)"
    )
